"""Strong α gate via OBSTACLE POSITION variation.

The theoretical question: does optimal α actually move when the
obstacle is in different places relative to the start->goal path?
- Obstacle on the direct path -> heavy intervention needed -> lower α
  (more conservative recovery)
- Obstacle off to the side -> light intervention -> higher α (more
  permissive)

If optimal α shifts across positions, we have a real state-conditional
signal that terrain failed to provide. If α is flat, this channel is
dead too and we ship the φ-only story.

One position per Python invocation (Isaac Sim won't cleanly rebuild
scenes within one process). Loop via the bash wrapper.

Run on labbox:
    bash phase6_obstacle_pos_gate.sh
"""
...
parser.add_argument("--task", default="Isaac-CBF-Adaptive-Go2-RMA-v0",
                    help="Use the RMA env -- obstacle position set via "
                         "cfg.actions.cbf_param.obstacle_xy below.")
parser.add_argument("--num_envs", type=int, default=256)
parser.add_argument("--eval_steps", type=int, default=1250)
parser.add_argument("--eval_eps_per_cell", type=int, default=256)
parser.add_argument("--obstacle_x", type=float, required=True)
parser.add_argument("--obstacle_y", type=float, required=True)
parser.add_argument("--alphas", type=float, nargs="+",
                    default=[0.5, 1.0, 1.5, 2.0, 2.5, 3.0, 4.0])
parser.add_argument("--phi_const", type=float, default=0.0)
parser.add_argument("--out_dir", default="phase6_obs_pos_gate_outputs")
...
def main():
    ...
    ox, oy = args_cli.obstacle_x, args_cli.obstacle_y
    tag = f"x{ox:+.1f}_y{oy:+.1f}"
    print()
    print("=" * 84)
    print(f"  OBSTACLE AT ({ox:+.2f}, {oy:+.2f})   (phi pinned at {args_cli.phi_const})")
    print("=" * 84)
    ...
    # pin obstacle at this cell's position; isolate this from other DR
    env_cfg.actions.cbf_param.obstacle_xy = (ox, oy)
    ...
    for alpha_v in args_cli.alphas:
        ...
        row = {"obs_x": ox, "obs_y": oy, "alpha": alpha_v, **m}
        rows.append(row)
    ...
    print()
    print(f"  best alpha @ ({ox:+.2f},{oy:+.2f}) [{verdict_tag}]: alpha={best['alpha']}  "
          f"(coll={best['collision_rate']:.2f}, reach={best['reach_rate']:.2f}, "
          f"int={best['intervention_mean']:.0f}, "
          f"track={best['tracking_err_mean']:.3f})")

    cells_path = os.path.join(args_cli.out_dir, f"obs_pos_{tag}_cells.csv")
    summ_path = os.path.join(args_cli.out_dir, f"obs_pos_{tag}_best.json")
    with open(cells_path, "w", newline="") as f:
        w = csv.DictWriter(f, fieldnames=list(rows[0].keys()))
        w.writeheader(); w.writerows(rows)
    with open(summ_path, "w") as f:
        json.dump({"obs_x": ox, "obs_y": oy, "tag": verdict_tag, **best}, f, indent=2)
