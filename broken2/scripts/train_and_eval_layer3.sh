#!/bin/bash
# Wk3 nrays128 (2026-05-16): RETRAIN with n_rays=128 + persist=50.
#
# Eval-only ablations on the pers checkpoint revealed:
#   - persist=10 → 50: NO EFFECT on baseline collision. Occlusions in our
#     scenes are shorter than 10 frames already; longer hold is no-op.
#   - n_rays=64 → 128: best fixed baseline collision dropped 0.594 → 0.455
#     (−13.9pp). Ray-gap angular sparsity was a real bottleneck — at 64
#     rays angular resolution is 5.625°/ray, small obstacles at distance
#     subtend less than that. 128 rays → 2.8°/ray, narrow obstacles
#     always seen.
#
# BR collision was eval-only-worse (0.586 → 0.620) because its policy
# was trained against n_rays=64 perception. Retraining with n_rays=128
# should let BR's adaptive params take advantage of the cleaner
# perception — hypothesis is BR collision drops to ≤ best baseline's
# 0.455, finally giving the architecture room to differentiate.
#
# Single-variable change from pers (training-side): SHIELD_N_RAYS_DEFAULT
# 64 → 128. Also keeps persist=50 from the prior eval-only test
# (no-op, but consistent).
#
# Cost trade-off: stuck_rate went UP slightly with more rays (baselines
# saw ~10pp jump in stuck-near-obstacle because more obstacles are
# perceived = more cautious paths). Combined fall+stuck-collision roughly
# flat. Pure collision (the hardware-deploy-critical metric) is what
# we're optimizing here.
#
...
#   tmux new -s wk3nrays128
#   cd ~/Desktop/safety-go2/IsaacLab && \
#   ~/Desktop/safety-go2/scripts/train_and_eval_layer3.sh 2>&1 | tee logs/train_and_eval_wk3nrays128.log
#
...
grep -q "CbfGo2EnvCfg_LAYER3" source/isaaclab_tasks/isaaclab_tasks/manager_based/safety/cbf_go2/cbf_go2_env_cfg.py \
  && echo "  ✓ CbfGo2EnvCfg_LAYER3 config present" \
  || { echo "  ✗ Layer 3 config missing — sync env_cfg.py"; exit 1; }
grep -q "Isaac-CBF-Go2-RMA-Layer3-v0" source/isaaclab_tasks/isaaclab_tasks/manager_based/safety/cbf_go2/__init__.py \
  && echo "  ✓ Isaac-CBF-Go2-RMA-Layer3-v0 task registered" \
...
./isaaclab.sh -p scripts/reinforcement_learning/rsl_rl/train.py \
  --task Isaac-CBF-Go2-RMA-Layer3-v0 \
...
./isaaclab.sh -p ~/Desktop/safety-go2/scripts/diagnose_phi_corr.py \
  ...
  --priv_dim 16 \
  --use_locked \
  --output diagnose_phi_corr_wk3nrays128.json \
  --headless
...
./isaaclab.sh -p ~/Desktop/safety-go2/scripts/diagnose_alpha_corr.py \
  ...
  --num_envs 256 --rollout_steps 100 \
  --priv_dim 16 \
  --output diagnose_alpha_corr_wk3nrays128.json \
  --headless
...
./isaaclab.sh -p ~/Desktop/safety-go2/scripts/probe_z_linear.py \
  ...
  --checkpoint "$CKPT" \
  --num_envs 256 --rollout_steps 100 \
  --output probe_z_linear_wk3nrays128.json \
  --headless
...
./isaaclab.sh -p ~/Desktop/safety-go2/scripts/diagnose_temporal_grid.py \
  ...
  --num_envs 256 --rollout_steps 100 \
  --cbf_state_dim 0 \
  --output diagnose_temporal_grid_wk3nrays128.json \
  --headless
...
EVAL_OUT="logs/baseline_eval_wk3nrays128_indist"
...
echo "α-corr JSON:           diagnose_alpha_corr_wk3nrays128.json"
echo "φ-corr JSON:           diagnose_phi_corr_wk3nrays128.json"
echo "Linear probe JSON:     probe_z_linear_wk3nrays128.json"
echo "Temporal-grid JSON:    diagnose_temporal_grid_wk3nrays128.json"
echo "Eval CSV:              logs/baseline_eval_wk3nrays128_indist/baseline.csv    ← Wk3 v1 HEADLINE"
echo ""
