...
echo "================================================================"
echo "Layer 2 (RMA split-encoder + 45% adversarial planner) pipeline"
...
grep -q "CbfGo2EnvCfg_LAYER2" source/isaaclab_tasks/isaaclab_tasks/manager_based/safety/cbf_go2/cbf_go2_env_cfg.py \
  && echo "  ✓ CbfGo2EnvCfg_LAYER2 config present" \
  || { echo "  ✗ HARD_PLANNER config missing"; exit 1; }
grep -q "Isaac-CBF-Go2-RMA-Layer2-v0" source/isaaclab_tasks/isaaclab_tasks/manager_based/safety/cbf_go2/__init__.py \
  && echo "  ✓ Isaac-CBF-Go2-RMA-Layer2-v0 task registered" \
  || { echo "  ✗ V32 task not registered in __init__.py"; exit 1; }

# ---------- PHASE 1: PPO TRAINING ----------
echo ""
echo "================================================================"
echo "[1/2] PPO TRAINING: ${CBF_ITERATIONS:-1500} iters, 4096 envs"
...
./isaaclab.sh -p ~/Desktop/safety-go2/scripts/diagnose_phi_corr.py \
  ...
  --priv_dim 16 \
  --use_locked \
  --output diagnose_phi_corr_v22.json \
  --headless
...
./isaaclab.sh -p ~/Desktop/safety-go2/scripts/diagnose_alpha_corr.py \
  ...
  --num_envs 256 --rollout_steps 100 \
  --priv_dim 16 \
  --output diagnose_alpha_corr_v22.json \
  --headless
...
./isaaclab.sh -p ~/Desktop/safety-go2/scripts/probe_z_linear.py \
  ...
  --checkpoint "$CKPT" \
  --num_envs 256 --rollout_steps 100 \
  --output probe_z_linear_v22.json \
  --headless
...
./isaaclab.sh -p ~/Desktop/safety-go2/scripts/diagnose_temporal_grid.py \
  ...
  --num_envs 256 --rollout_steps 100 \
  --cbf_state_dim 0 \
  --output diagnose_temporal_grid_v22.json \
  --headless
...
EVAL_OUT="logs/baseline_eval_v22_indist"
...
echo "α-corr JSON:           diagnose_alpha_corr_v22.json"
echo "φ-corr JSON:           diagnose_phi_corr_v22.json"
echo "Linear probe JSON:     probe_z_linear_v22.json"
echo "Temporal-grid JSON:    diagnose_temporal_grid_v22.json"
echo "Eval CSV:              logs/baseline_eval_v22_indist/baseline.csv    ← v22 HEADLINE"
echo ""
