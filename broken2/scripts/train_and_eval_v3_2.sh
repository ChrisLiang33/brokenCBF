#!/bin/bash
# v3.2 (2026-05-11): RMA-style architecture (from v3.1) +
...
#
# v3.2 fixes the data, not the loss:
...
#   rsync -av ~/Desktop/safety-go2/scripts/train_and_eval_v3_2.sh \
#       chrisliang@130.64.84.163:Desktop/safety-go2/scripts/
#
# Usage on lab box (in tmux):
#   tmux new -s v32
#   cd ~/Desktop/safety-go2/IsaacLab && \
#   ~/Desktop/safety-go2/scripts/train_and_eval_v3_2.sh 2>&1 | tee logs/train_and_eval_v3_2.log

set -e

cd ~/Desktop/safety-go2/IsaacLab

# v3.2: aux loss OFF. Adversarial planner provides the dense signal.
AUX_COEF=0.0

echo "================================================================"
echo "v3.2 (RMA split-encoder + 45% adversarial planner) pipeline"
echo "Started: $(date '+%Y-%m-%d %H:%M:%S')"
echo "================================================================"

# Pre-flight: confirm v3.2 changes are in place
echo ""
...
for TASK in "${EVAL_TASKS[@]}"; do
  ...
  OUT="logs/baseline_eval_v3_2_${TAG}"
  echo ""
...
./isaaclab.sh -p ~/Desktop/safety-go2/scripts/diagnose_alpha_corr.py \
  ...
  --output diagnose_alpha_corr_v3_2.json \
  --headless
...
./isaaclab.sh -p ~/Desktop/safety-go2/scripts/probe_z_linear.py \
  ...
  --checkpoint "$CKPT" \
  --num_envs 256 --rollout_steps 100 \
  --output probe_z_linear_v3_2.json \
  --headless
...
echo "Eval CSVs:         logs/baseline_eval_v3_2_{indist,HeavyCOM}/"
echo "α-corr JSON:       diagnose_alpha_corr_v3_2.json"
echo "Linear probe JSON: probe_z_linear_v3_2.json"
echo "Z diagnostic JSON: diagnose_z_v3_2.json"
echo ""
