#!/bin/bash
# v2.15 = v2.11 stack + cylinder-only obstacle pool + QP-side obstacle-position
...
#
#   v2.15 hypothesis: with QP-side obstacle-position noise, `a`/`c` come
...
#
# v2.15 changes (delta from v2.11):
#
#   Code changes:
...
#
# Usage on lab box:
#   ./scripts/train_and_eval_v215.sh 2>&1 | tee logs/train_and_eval_v215.log
#
# IMPORTANT: in tmux pane B during training, run the health watcher:
#   bash ~/Desktop/safety-go2/scripts/watch_training_health.sh \
#     ~/Desktop/safety-go2/IsaacLab/logs/train_and_eval_v215.log
#   Health log appended to: train_and_eval_v215.health.log
...
echo "================================================================"
echo "v2.15 (cylinder pool + QP-side noise injection + 6-planner mix) pipeline"
...
for ((i=0; i<${#EVAL_TASKS[@]}; i+=2)); do
  ...
  TAG1=$(task_tag "$TASK1")
  OUT1="logs/baseline_eval_v215_${TAG1}"
  ...
  if [ -n "$TASK2" ]; then
    ...
    TAG2=$(task_tag "$TASK2")
    OUT2="logs/baseline_eval_v215_${TAG2}"
...
# ---------- DUAL-REGIME DIAGNOSTIC ----------
# v2.15: SKIPPED (saves ~50 min wall clock).
# v2.12 mid-switch result was directionally same as locked, not load-bearing
# for the ship/no-ship decision. Bring back if v2.15 lands and we want
# the apples-to-apples vs v2.6 paper baseline regime.
echo ""
echo "================================================================"
echo "[3a/3] DUAL-REGIME DIAGNOSTIC SKIPPED (v2.15 trim)"
echo "================================================================"
...
./isaaclab.sh -p ~/Desktop/safety-go2/scripts/eval_baseline.py \
  ...
  --modes BR,Bf-alpha \
  --checkpoint "$CKPT" \
  --output_dir "logs/baseline_eval_v215_bfalpha_indist" \
  --headless > "logs/baseline_eval_v215_bfalpha_indist.stdout.log" 2>&1 &
PID1=$!
...
./isaaclab.sh -p ~/Desktop/safety-go2/scripts/eval_baseline.py \
  ...
  --modes BR,Bf-alpha \
  --checkpoint "$CKPT" \
  --output_dir "logs/baseline_eval_v215_bfalpha_DensePack" \
  --headless > "logs/baseline_eval_v215_bfalpha_DensePack.stdout.log" 2>&1 &
PID2=$!

wait $PID1; echo "  >>> [Bf-α v0] done at $(date '+%H:%M:%S')"
wait $PID2; echo "  >>> [Bf-α DensePack] done at $(date '+%H:%M:%S')"

# Pair 2: Bf-φ on HighActuationNoise (v2.15 NEW — proper φ axis), Bf-a on NoisyPerception
echo ""
echo "  >>> launch [Bf-φ HighActuationNoise] + [Bf-a NoisyPerception]  ($(date '+%H:%M:%S'))"
./isaaclab.sh -p ~/Desktop/safety-go2/scripts/eval_baseline.py \
  --task "Isaac-CBF-Go2-HighActuationNoise-v0" \
  --num_envs 64 --steps_per_config 1500 \
  --modes BR,Bf-phi \
  --checkpoint "$CKPT" \
  --output_dir "logs/baseline_eval_v215_bfphi_HighActuationNoise" \
  --headless > "logs/baseline_eval_v215_bfphi_HighActuationNoise.stdout.log" 2>&1 &
PID1=$!
...
./isaaclab.sh -p ~/Desktop/safety-go2/scripts/eval_baseline.py \
  ...
  --modes BR,Bf-a \
  --checkpoint "$CKPT" \
  --output_dir "logs/baseline_eval_v215_bfa_NoisyPerception" \
  --headless > "logs/baseline_eval_v215_bfa_NoisyPerception.stdout.log" 2>&1 &
PID2=$!

wait $PID1; echo "  >>> [Bf-φ HighActuationNoise] done at $(date '+%H:%M:%S')"
wait $PID2; echo "  >>> [Bf-a NoisyPerception] done at $(date '+%H:%M:%S')"

# Pair 3: Bf-c on HeavyCOM + RadiusError (v2.15 NEW — proper c axis)
echo ""
echo "  >>> launch [Bf-c HeavyCOM] + [Bf-c RadiusError]  ($(date '+%H:%M:%S'))"
./isaaclab.sh -p ~/Desktop/safety-go2/scripts/eval_baseline.py \
  --task "Isaac-CBF-Go2-HeavyCOM-v0" \
  --num_envs 64 --steps_per_config 1500 \
  --modes BR,Bf-c \
  --checkpoint "$CKPT" \
  --output_dir "logs/baseline_eval_v215_bfc_HeavyCOM" \
  --headless > "logs/baseline_eval_v215_bfc_HeavyCOM.stdout.log" 2>&1 &
PID1=$!

sleep 30
./isaaclab.sh -p ~/Desktop/safety-go2/scripts/eval_baseline.py \
  --task "Isaac-CBF-Go2-RadiusError-v0" \
  --num_envs 64 --steps_per_config 1500 \
  --modes BR,Bf-c \
  --checkpoint "$CKPT" \
  --output_dir "logs/baseline_eval_v215_bfc_RadiusError" \
  --headless > "logs/baseline_eval_v215_bfc_RadiusError.stdout.log" 2>&1 &
PID2=$!

wait $PID1; echo "  >>> [Bf-c HeavyCOM] done at $(date '+%H:%M:%S')"
wait $PID2; echo "  >>> [Bf-c RadiusError] done at $(date '+%H:%M:%S')"
...
echo "Headline 10-eval CSVs (v2.15 added HighActuationNoise + RadiusError):"
ls -la logs/baseline_eval_v215_indist/baseline.csv \
       logs/baseline_eval_v215_DensePack/baseline.csv \
       logs/baseline_eval_v215_Slippery/baseline.csv \
       logs/baseline_eval_v215_HighDisturbance/baseline.csv \
       logs/baseline_eval_v215_HeavyCOM/baseline.csv \
       logs/baseline_eval_v215_FastObstacles/baseline.csv \
       logs/baseline_eval_v215_RealisticCompound/baseline.csv \
       logs/baseline_eval_v215_NoisyPerception/baseline.csv \
       logs/baseline_eval_v215_HighActuationNoise/baseline.csv \
       logs/baseline_eval_v215_RadiusError/baseline.csv 2>/dev/null
echo ""
echo "Table-2 ablation CSVs (Bf-φ → HighActuationNoise, Bf-c → HeavyCOM + RadiusError):"
ls -la logs/baseline_eval_v215_bf*/baseline.csv 2>/dev/null
echo "================================================================"
