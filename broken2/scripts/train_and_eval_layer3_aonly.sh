#!/bin/bash
# Wk3 v2 a-only (2026-05-15): RELEASE ADDITIVE-SLACK `a` ONLY.
#
# Wk3 v1 pers landed at BR ≈ baseline parity on collision (0.586 vs 0.594)
# under realistic LiDAR perception with persistence. LAYER3_TRACKER and
# LAYER3_FULL both regressed because synthetic tracker noise on v_obs was
# too aggressive — PPO dropped phi to escape the noisy L_f h gradient.
#
# This iteration drops synthetic tracker noise entirely and releases ONLY
# the additive-slack `a` parameter (h-shift `c` stays frozen at 0).
# Single-variable change from Wk3 v1 pers: freeze_a_value = None.
#
# Hypothesis: adaptive `a` can absorb the residual perception uncertainty
# inherent in shield_v0c (cluster centroid jitter as the robot moves,
# persistence-cache drift during occlusion, per-episode hidden DR state)
# without needing synthetic tracker noise. If `a` finds useful work and
# drops BR collision below pers's 0.586, that's the paper claim landing.
#
# Decision gates:
#   - mean(a) bounded (not pegged at 0 OR at 3.0)
#   - a_std > 0 (some variation across envs)
#   - Pearson(a, |applied_force| or |com_offset| or σ_act) > 0.15
#   - BR combined fall+stuck-collision <= best fixed-(α, φ) baseline
#   - Encoder R²(COM, σ) preserved
#
# Fallback if `a` stays flat: bump obstacle max speed from 0.2 → 0.5 or
# 1.0 m/s. 4cm of persistence drift at 0.2 m/s spawn speed may not be
# painful enough for PPO to discover `a`'s role. 20cm drift at 1.0 m/s
# would be.
#
...
#   ~/Desktop/safety-go2/scripts/train_and_eval_layer3.sh 2>&1 | tee logs/train_and_eval_wk3aonly.log
#
...
grep -q "CbfGo2EnvCfg_LAYER3_A_ONLY" source/isaaclab_tasks/isaaclab_tasks/manager_based/safety/cbf_go2/cbf_go2_env_cfg.py \
  && echo "  ✓ CbfGo2EnvCfg_LAYER3_A_ONLY config present" \
  || { echo "  ✗ Layer 3 config missing — sync env_cfg.py"; exit 1; }
grep -q "Isaac-CBF-Go2-RMA-Layer3-AOnly-v0" source/isaaclab_tasks/isaaclab_tasks/manager_based/safety/cbf_go2/__init__.py \
  && echo "  ✓ Isaac-CBF-Go2-RMA-Layer3-AOnly-v0 task registered" \
...
./isaaclab.sh -p scripts/reinforcement_learning/rsl_rl/train.py \
  --task Isaac-CBF-Go2-RMA-Layer3-AOnly-v0 \
...
./isaaclab.sh -p ~/Desktop/safety-go2/scripts/diagnose_phi_corr.py \
  ...
  --priv_dim 16 \
  --use_locked \
  --output diagnose_phi_corr_wk3aonly.json \
  --headless
...
./isaaclab.sh -p ~/Desktop/safety-go2/scripts/diagnose_alpha_corr.py \
  ...
  --num_envs 256 --rollout_steps 100 \
  --priv_dim 16 \
  --output diagnose_alpha_corr_wk3aonly.json \
  --headless
...
./isaaclab.sh -p ~/Desktop/safety-go2/scripts/probe_z_linear.py \
  ...
  --checkpoint "$CKPT" \
  --num_envs 256 --rollout_steps 100 \
  --output probe_z_linear_wk3aonly.json \
  --headless
...
./isaaclab.sh -p ~/Desktop/safety-go2/scripts/diagnose_temporal_grid.py \
  ...
  --num_envs 256 --rollout_steps 100 \
  --cbf_state_dim 0 \
  --output diagnose_temporal_grid_wk3aonly.json \
  --headless
...
EVAL_OUT="logs/baseline_eval_wk3aonly_indist"
...
echo "α-corr JSON:           diagnose_alpha_corr_wk3aonly.json"
echo "φ-corr JSON:           diagnose_phi_corr_wk3aonly.json"
echo "Linear probe JSON:     probe_z_linear_wk3aonly.json"
echo "Temporal-grid JSON:    diagnose_temporal_grid_wk3aonly.json"
echo "Eval CSV:              logs/baseline_eval_wk3aonly_indist/baseline.csv    ← Wk3 v1 HEADLINE"
echo ""
