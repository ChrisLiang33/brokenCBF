#!/usr/bin/env python3
"""Within-distribution state-conditioning diagnostic for the additive
slack `a` (CBF action dim index 2).

Mirrors diagnose_alpha_corr.py but targets `a` instead of α. The headline
question is whether Pearson(`a`, |tracking_err|) is positive (more push →
more safety margin, the design hypothesis) or negative (`a` spikes
downward under disturbance, suggesting the L1 tax is fighting the
...
Usage on lab box:
  ./isaaclab.sh -p ~/Desktop/safety-go2/scripts/diagnose_a_corr.py \\
    ...
    --priv_dim 31 \\
    --output diagnose_a_corr_wk3pusha.json \\
    --headless
"""
...
print(f"[diagnose_a_corr] starting AppLauncher...", flush=True)
t0 = time.time()
app_launcher = AppLauncher(args)
simulation_app = app_launcher.app
print(f"[diagnose_a_corr] AppLauncher ready in {time.time()-t0:.1f}s", flush=True)
...
# Matches cbf_go2_env._cbf_filter WIDE_PARAM_RANGES branch for dim 2:
#   a_param = (tanh(raw[:, 2]) + 1) * 0.5 * 3.0   →   [0.0, 3.0]
A_MIN = 0.0
A_MAX = 3.0
...
def main() -> int:
    ...
    print(f"[diagnose_a_corr] task={args.task}, num_envs={args.num_envs}, "
    ...
    print(f"[diagnose_a_corr] runner loaded.", flush=True)

    obs, _ = env.reset()

    N = args.num_envs
    S = args.rollout_steps
    a_history = np.zeros((S, N), dtype=np.float32)
    priv_history = np.zeros((S, N, args.priv_dim), dtype=np.float32)

    # For within-episode (per-step) correlations, we want time-series, not
    # per-env aggregates. We'll compute BOTH: per-env mean (between-env
    # correlation) and per-step concatenated (within-episode correlation).
    for step in range(S):
        with torch.no_grad():
            raw_action = policy(obs)
            a_phys = (torch.tanh(raw_action[:, 2]) + 1.0) * 0.5 * (A_MAX - A_MIN) + A_MIN
            a_history[step] = a_phys.cpu().numpy()
        ...
        if step % 20 == 0 or step == S - 1:
            print(f"[diagnose_a_corr] step {step:>3}/{S}  "
                  f"a mean={a_phys.mean().item():.3f}  "
                  f"a std={a_phys.std().item():.3f}", flush=True)

    # ── Aggregate per env (between-env correlations) ──
    a_per_env = a_history.mean(axis=0)               # (N,)
    priv_per_env = priv_history.mean(axis=0)         # (N, priv_dim)
    a_within_env_std = a_history.std(axis=0).mean()  # avg over envs

    # ── Concatenate all (env, step) (within-episode correlations) ──
    a_flat = a_history.reshape(-1)                                  # (S*N,)
    priv_flat = priv_history.reshape(-1, args.priv_dim)             # (S*N, priv_dim)
    ...
    for name in ["friction", "base_mass", "base_height", "actuation_noise_sigma"]:
        ...
        correlations_between_env[name] = pearson_corr(feat_env, a_per_env)
        correlations_within_episode[name] = pearson_corr(feat_flat, a_flat)
        detail_scalars[name] = {
            "mean": float(feat_env.mean()),
            "std": float(feat_env.std()),
        }

    # Vector features → correlate with norm of the vector
    for name in ["applied_force", "applied_torque", "tracking_err", "base_ang_vel", "com_offset"]:
        ...
        correlations_between_env[f"|{name}|"] = pearson_corr(norm_env, a_per_env)
        correlations_within_episode[f"|{name}|"] = pearson_corr(norm_flat, a_flat)
    ...
    stats = {
        ...
        "a_population_mean": float(a_per_env.mean()),
        "a_population_std": float(a_per_env.std()),
        "a_within_env_std_mean": float(a_within_env_std),
        "correlations_with_a_between_env": {
            k: round(v, 4) for k, v in correlations_between_env.items()
        },
        "correlations_with_a_within_episode": {
    ...
    print("", flush=True)
    print("=" * 70, flush=True)
    print(f"`a` correlation diagnostic — {args.task}", flush=True)
    print("=" * 70, flush=True)
    print(f"  `a` population: mean={stats['a_population_mean']:.3f}  "
          f"between-env std={stats['a_population_std']:.3f}  "
          f"within-env std={stats['a_within_env_std_mean']:.3f}", flush=True)
    print("", flush=True)
    print(f"  Pearson(`a`, feature) WITHIN-episode  (per-step, the gate we care about):", flush=True)
    ...
    print("", flush=True)
    print(f"  Pearson(`a`, feature) BETWEEN-env (per-env mean):", flush=True)
...
if __name__ == "__main__":
    ...
    try:
        ...
        import traceback
        print(f"[diagnose_a_corr] FATAL: {e}", flush=True)
        traceback.print_exc()
