#!/bin/bash
# Wk3 v2 / Layer 3 Full (2026-05-15): tracker noise + release a + release c +
# symmetric c range. This is the empirical foundation for the paper's
# Table 2 claim — robust teacher with all 5 adaptive CBF params vs fixed
# (α, φ) baselines under realistic perception + tracker noise.
#
# Coordinated changes from Layer 3 (gr60 prerequisite must clear first):
#   1. v_obs_tracker_enabled = True
#      Synthetic classical-tracker noise on the obstacle velocity vector
#      the QP reads for L_f h:
#        - Speed-scaled Gaussian jitter σ_v = 0.05 + 0.15·‖v_true‖
#        - Speed-scaled dropout P = 0.01 + 0.05·‖v_true‖, duration [5, 15]
#          steps, v_obs FROZEN at last-observed during dropout (not zero —
#          a real Kalman tracker propagates the constant-velocity
#          estimate)
#        - Deterministic 2-step latency (40ms at 50 Hz CBF rate)
#   2. freeze_a_value = None
#      Additive-slack `a` released. Range [0.0, 3.0] (one-sided —
#      negative `a` would be suicidal).
#   3. freeze_c_value = None  +  c_param_range = (-0.20, 0.20)
#      H-shift `c` released AND clamp symmetric. Replaces legacy
#      [C_MIN=0.10, 1.0] non-negative range. Required because the
#      shield_v0c+rfit perception model OVER-estimates obstacle radii
#      (max_hit_to_centroid + 0.10m safety margin), so perceived h is
#      smaller than true h. Policy needs `c < 0` to push the QP boundary
#      back out toward true geometry.
#
# Decision criteria for Wk3 Step 5:
#   - Pearson(c, perception_bias_axis) < -0.20   ← c compensates the
#     rfit over-estimate
#   - Pearson(a, obstacle_speed_max) > +0.20      ← a absorbs tracker noise
#   - mean(c) < 0                                  ← directionally correct
#   - mean(a) bounded (NOT pegged at 3.0)         ← didn't saturate
#   - Pearson(φ, σ_actuation) > 0.20              ← clean φ-σ coupling
#     finally lands (had partial signal at +0.098 under broken perception)
#   - Encoder R²(COM/σ) preserved                  ← representation
#     wasn't compromised by harder task
#   - Robust teacher combined fall+stuck BEATS best fixed-(α, φ) baseline
#     on the same Layer 3 Full task  ← the paper claim's empirical
#     foundation
#
# Sync before launch:
...
#   rsync -av ~/Desktop/safety-go2/scripts/train_and_eval_layer3_full.sh \
#       chrisliang@130.64.84.163:Desktop/safety-go2/scripts/
#
# Usage on lab box:
#   tmux new -s wk3full
#   cd ~/Desktop/safety-go2/IsaacLab && \
#   ~/Desktop/safety-go2/scripts/train_and_eval_layer3_full.sh 2>&1 | tee logs/train_and_eval_wk3full.log
#
# ─────────────────── HISTORICAL CONTEXT (pre-Wk3 Step 5) ───────────────────
#
...
#   tmux new -s wk3full
#   cd ~/Desktop/safety-go2/IsaacLab && \
#   ~/Desktop/safety-go2/scripts/train_and_eval_layer3.sh 2>&1 | tee logs/train_and_eval_wk3full.log
#
...
echo "================================================================"
echo "Layer 3 Full / Wk3 v2 (perception + tracker noise + release a + release c, symmetric c)"
...
grep -q "CbfGo2EnvCfg_LAYER3_FULL" source/isaaclab_tasks/isaaclab_tasks/manager_based/safety/cbf_go2/cbf_go2_env_cfg.py \
  && echo "  ✓ CbfGo2EnvCfg_LAYER3_FULL config present" \
  || { echo "  ✗ Layer 3 config missing — sync env_cfg.py"; exit 1; }
grep -q "Isaac-CBF-Go2-RMA-Layer3-Full-v0" source/isaaclab_tasks/isaaclab_tasks/manager_based/safety/cbf_go2/__init__.py \
  && echo "  ✓ Isaac-CBF-Go2-RMA-Layer3-Full-v0 task registered" \
...
./isaaclab.sh -p scripts/reinforcement_learning/rsl_rl/train.py \
  --task Isaac-CBF-Go2-RMA-Layer3-Full-v0 \
...
./isaaclab.sh -p ~/Desktop/safety-go2/scripts/diagnose_phi_corr.py \
  ...
  --priv_dim 16 \
  --use_locked \
  --output diagnose_phi_corr_wk3full.json \
  --headless
...
./isaaclab.sh -p ~/Desktop/safety-go2/scripts/diagnose_alpha_corr.py \
  ...
  --num_envs 256 --rollout_steps 100 \
  --priv_dim 16 \
  --output diagnose_alpha_corr_wk3full.json \
  --headless
...
./isaaclab.sh -p ~/Desktop/safety-go2/scripts/probe_z_linear.py \
  ...
  --checkpoint "$CKPT" \
  --num_envs 256 --rollout_steps 100 \
  --output probe_z_linear_wk3full.json \
  --headless
...
./isaaclab.sh -p ~/Desktop/safety-go2/scripts/diagnose_temporal_grid.py \
  ...
  --num_envs 256 --rollout_steps 100 \
  --cbf_state_dim 0 \
  --output diagnose_temporal_grid_wk3full.json \
  --headless
...
EVAL_OUT="logs/baseline_eval_wk3full_indist"
...
echo "α-corr JSON:           diagnose_alpha_corr_wk3full.json"
echo "φ-corr JSON:           diagnose_phi_corr_wk3full.json"
echo "Linear probe JSON:     probe_z_linear_wk3full.json"
echo "Temporal-grid JSON:    diagnose_temporal_grid_wk3full.json"
echo "Eval CSV:              logs/baseline_eval_wk3full_indist/baseline.csv    ← Wk3 v1 HEADLINE"
echo ""
